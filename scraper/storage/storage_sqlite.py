...
DB_DEFAULT = Path(__file__).parent.parent.parent / "articles.db"
...
_MIGRATIONS = [
    ...
    "ALTER TABLE article_analysis ADD COLUMN notified_at TEXT",
    "ALTER TABLE scheduler_settings ADD COLUMN notify_always INTEGER NOT NULL DEFAULT 0",
]


class SQLiteStorage(StorageBackend):
    """SQLite-backed storage. Supports :memory: for tests."""
    ...
    def _ensure_tables(self) -> None:
        with self._conn() as conn:
            ...
            conn.execute(_CREATE_ANALYSIS_TABLE)
            conn.execute(_CREATE_CRAWL_SESSIONS)
            conn.execute(_CREATE_CRAWL_SESSION_ARTICLES)
            conn.execute(_CREATE_SCHEDULER_SETTINGS)
        ...
        *,
        ...
        with self._conn() as conn:
            try:
                ...
                conn.execute(_POPULATE_FTS, (row_id,))
                return row_id
    ...
    def save_analysis(self, result: "AnalysisResult") -> int | None:
        from pipeline.models import AnalysisResult  # noqa: F401 — local to avoid circular import
        with self._conn() as conn:
            try:
                cur = conn.execute(
                    """
